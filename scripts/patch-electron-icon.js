 ...
 *
 * Patches the Electron dev binary with ZaloCRM's icon and version metadata.
...
async function main() {
  ...
  try {
    ...
    await rcedit(electronExePath, {
      icon: iconPath,
      'version-string': {
        ProductName:     pkg.build?.productName || 'ZaloCRM',
        FileDescription: pkg.build?.productName || 'ZaloCRM',
        CompanyName:     'ZaloCRM',
      },
    ...
    console.log('[patch-icon] ✅ electron.exe patched with ZaloCRM icon');
  } catch (err) {
